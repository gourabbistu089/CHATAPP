...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (avatarLocalPath) {
   ...
   // Set a profile photo link based on the user's gender
   const girlyaboy = (gender === "male" ? "boy" : "girl");
   const profilePhotoLink = `https://avatar.iran.liara.run/public/${girlyaboy}`;
  ...
  try {
    ...
    const newUser = await User.create({
      fullname,
      avatar: avatar?.url || profilePhotoLink ,
      email,
      password,
    ...
    // 6. Set access and refresh tokens in cookies and return response
    return res
    .status(201)
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!isPasswordCorrect) {
    throw new ApiError(401, "Password is incorrect"); //  401 is used for unauthorized access
